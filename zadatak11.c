...
int main()
{
    ...
    while (!feof(f))
    {
        ...
        key = Key(NameOfCountry);
        AddCountry(NameOfCountry, NameOfFile, &Hash[key]);
    }
    fclose(f);
    //PrintCountries(Head.next);

    printf("List of countries from file:\n");
    for (i = 0; i < N; i++)
    {
        printf("Hash[%d]:\n", i);
    ...
    key = Key(NameOfCountry);
    P = SearchCountry(&Hash[key], NameOfCountry);
    if (P == NULL)
    {
...
int SearchCity(Tree S, int residents)
{
    if (S == NULL)
        return 0;
    SearchCity(S->L, residents);
    if (S->residents >= residents) {
        printf("%s %d\n", S->name, S->residents);
        SearchCity(S->R, residents);
    }
    else
        printf("City with the wanted population doesn't exist in this file.\n");
